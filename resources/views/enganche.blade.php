...
<div class="logo-container">
    ...
    <div class="section">
        ...
        <p class= p2>Teléfono: 492-161-68-35</p2>
...
<div class="section right-align" style="text-align: justify;">
@foreach ($venta->contacto->ventas as $venta)
@if ($loop->first) <!-- Verificar si es la primera iteración -->
        @if($venta->lote)
            <p>
            <span class="label">Lote:</span> 
            <span style="text-decoration: underline;">{{ $venta->lote->manzana ? $venta->lote->manzana->nombre : 'No disponible' }} - {{ $venta->lote->lote ? $venta->lote->lote : 'No disponible' }} - 
                @if($venta->lote->area)
        ...
        @else
            <p><span class="label">Lote:</span>No asignado</p>
        @endif
    @endif
    @endforeach
</div>
...
<div class="section right-align" style="text-align: justify;">
    ...
    <p><span class="label">Total Venta:</span>$ {{ number_format($venta->precio_venta_final, 2) }}</p>
    <p><span class="label">Cuota:</span> Enganche</p>
</div>
...
<table class="table">
    ...
    <tbody>
    <tr>
    <td>CUOTA-ENGANCHE</td>
    <td>$ {{ number_format($venta->enganche, 2) }}</td> <!-- Agregar coma como separador de miles -->
</tr>
<tr>
    <td>TOTAL</td>
    <td>$ {{ number_format($venta->enganche, 2) }}</td> <!-- Agregar coma como separador de miles -->
